
export const staffData = [
    { id: 1, name: 'Dr. N. Palanivel', role: 'Professor & HOD	', image: '/images/faculity_photo/Palani.jpg' },
    { id: 2, name: 'Mr.U.Muruganantham', role: 'Assistant Professor', image: '/images/faculity_photo/muruganantham.jpg' },
    { id: 3, name: 'Kumaraguru', role: 'Assistant Professor', image: 'https://placehold.co/400x400?text=Kumaraguru&font=roboto' },
    { id: 4, name: 'Mrs.K.C Nithyasree', role: 'Assistant Professor', image: '/images/faculity_photo/Nithasri.jpg' },
    { id: 5, name: 'Mrs.K.Kavitha', role: 'Assistant Professor', image: '/images/faculity_photo/Kavitha.jpg' },
    { id: 6, name: 'Ms..A.Sheerin', role: 'Assistant Professor', image: '/images/faculity_photo/sheerin.jpg' },
    { id: 6, name: 'Ms. S.Adolphine Shyni', role: 'Assistant Professor', image: 'https://placehold.co/400x400?text=Adolphine+Shyni&font=roboto' },

  ];